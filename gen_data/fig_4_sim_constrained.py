...
MU_DD = np.array([0.45,0.55])
LAMBDA_DD = np.array([0.35,0.4])
MU_UU = np.array([0.45,0.5])
        ...
        for mu_u in MU_UU:
            ...
            for lambda_d in LAMBDA_DD:
                ...
                for lambda_u in LAMBDA_UU:
                    ...
                    if not os.path.isdir('sim/sim11/'+str(mu_d)+'_'+str(lambda_d)+'_'+str(mu_u)+'_'+str(lambda_u)):
                        os.mkdir('sim/sim11/'+str(mu_d)+'_'+str(lambda_d)+'_'+str(mu_u)+'_'+str(lambda_u))
                    queue_dict_u = {'queue_max_size':1, 'admission_prob':lambda_u,\
		                          'service_prob':1.0, 'fifo':False}
                    ...
                    for i in range(NO_OF_SIMULATIONS):
                        ...
                        system.evolve()
                        system.dump_json('sim/sim11/'+str(mu_d)+'_'+str(lambda_d)+\
...
def compile_data_from_folders():
        ...
        for lambda_d in LAMBDA_DD:
            ...
            for mu_u in MU_UU:            
                ...
                for lambda_u in LAMBDA_UU:
                    ...
                    cost1 = []
                    for i in range(NO_OF_SIMULATIONS):
                        with open('sim/sim11/'+str(mu_d)+'_'+str(lambda_d)+'_'+str(mu_u)+'_'+str(lambda_u)+\
    ...
    file_name = "sim/sim11_data_055_10000x1000.json"
    with open(file_name, 'w') as file:
        json.dump(data1, file, ensure_ascii=False)
    print(pd.DataFrame(data1["mean"]))

if not os.path.isdir("sim/sim11"):
    os.mkdir("sim/sim11")
