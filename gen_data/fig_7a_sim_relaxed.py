...
from blocks.mdpsystem import MDPSystem
        ...
        for mu_u in MU_UU:
            ...
            for lambda_d in LAMBDA_DD:
                ...
                for lambda_u in LAMBDA_UU:
                    ...
                    if not os.path.isdir('sim/sim3/'+str(mu_d)+'_'+str(lambda_d)+'_'+str(mu_u)+'_'+str(lambda_u)):
                        os.mkdir('sim/sim3/'+str(mu_d)+'_'+str(lambda_d)+'_'+str(mu_u)+'_'+str(lambda_u))
                    queue_dict_u = {'queue_max_size':NO_OF_STEPS+1, 'admission_prob':lambda_u,\
		                      'service_prob':mu_u, 'fifo':True}
                    queue_dict_d = {'queue_max_size':NO_OF_STEPS+1, 'admission_prob':lambda_d,\
		                      'service_prob':mu_d, 'fifo':True}
                    for i in range(NO_OF_SIMULATIONS):
                        INIT_STATE = np.random.choice(S, p=STATIONARY_STATE_DIST[0])
                        system = MDPSystem(NO_OF_STEPS, P[0], R, queue_dict_u, queue_dict_d)
                        system.set_initial_state(INIT_STATE)
                        system.evolve()
                        system.dump_json('sim/sim3/'+str(mu_d)+'_'+str(lambda_d)+\
                            '_'+str(mu_u)+'_'+str(lambda_u)+'/test_'+str(i)+'.json')

def compile_data_from_folders():
    data1 = {}
    data1["mu_d"] = {"lambda_d":{"mu_u":"lambda_u"}}
    data1["mean"] = {}
    data1["std"] = {}
    #data1["cost"] = {}
    for mu_d in MU_DD:
        ...
        for lambda_d in LAMBDA_DD:
            ...
            for mu_u in MU_UU:            
                ...
                for lambda_u in LAMBDA_UU:
                    ...
                    cost1 = []
                    for i in range(NO_OF_SIMULATIONS):
                        with open('sim/sim3/'+str(mu_d)+'_'+str(lambda_d)+'_'+str(mu_u)+'_'+str(lambda_u)+\
